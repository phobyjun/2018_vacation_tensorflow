...
negative_samples = 8
# 상대경로를 사용하면 tensorboard 실행 시 지정한 LOG_DIR의 경로 아래의 상대경로에서 임베딩 기록을 찾기 때문에 오류가 발생한다.
LOG_DIR = "C:/Users/Junseok Yoon/Desktop/2018_vacation_tensorflow/Chapter_6/logs/word2vec_intro"
...
# Create two kinds of sentences - sequences of add and even digits
...
with tf.name_scope("embeddings"):
    embeddings = tf.Variable(tf.random_uniform([vocabulary_size, embedding_demension], -1.0, 1.0), name='embedding')
    # This is essentially a lookup table
    embed = tf.nn.embedding_lookup(embeddings, train_inputs)

# Create variables for the NCE loss
nce_weights = tf.Variable(tf.truncated_normal([vocabulary_size, embedding_demension],
                                              stddev=1.0 / math.sqrt(embedding_demension)))
